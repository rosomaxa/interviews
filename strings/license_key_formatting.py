...
each part has 2 characters except the first part as it could be shorter
as mentioned above.
Note:
...
class Solution(object):
    def licenseKeyFormatting(self, S, K):
        ...
        :rtype: str
        """
        plain_key = filter(lambda x: x != '-', S).upper()
        chunk = len(plain_key) % K or K
        result = [plain_key[:chunk]]

        while chunk < len(plain_key):
            this_part = plain_key[chunk:chunk + K]
            if this_part:
