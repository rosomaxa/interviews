...
class Solution(object):
    def wordsTyping(self, sentence, rows, cols):
        ...
        :rtype: int
        """
        single_line = ' '.join(sentence) + ' '
        counter = 0
        n = len(single_line)

        for i in xrange(rows):
            counter += cols
            this_line_end = counter % n
            # print single_line[:this_line_end]
            if single_line[this_line_end] == ' ':
                counter += 1
            else:
                while counter > 0 and single_line[(counter - 1) % n] != ' ':
                    counter -= 1
...
if __name__ == '__main__':
    ...
    assert Solution().wordsTyping(["abc", "de", "f"], 4, 6) == 2
