...
class Solution(object):
    ...
    def is_hex(self, num):
        return all(s in string.hexdigits for s in num)
    ...
    def validIPAddress(self, IP):
        ...
        if is_ipv6:
            max_ = int(math.pow(2, 16))
            checked = 0
            for num in ipv6:
                if num == '' or len(num) > 4:
                    return 'Neither'

                if not self.is_hex(num):
                    return 'Neither'

                if 0 <= int(num, 16) < max_:
                    checked += 1
...
if __name__ == '__main__':
    print Solution().validIPAddress('00.0.0.0')
